...
f_data <- populateGeneNames(quant_data) # fetching data from uniprot

```
```{r}
#| echo: false

f_data <- read.csv("data/feature_data.csv")
```


### Gene ID has multiple entries in cell
...
# fetching data for multiple accessions
mquant_data <- populateGeneNamesfromMultipleAccession(quant_data, "Accession", ",", information = information, countAccession = countAccession, getGeneUniProt = getGeneUniProt, getGeneInformationforMultipleAccession = getGeneInformationforMultipleAccession, verbose_ = FALSE)


quant_data <- mquant_data[, c(1, length(unlist(str_split(information, ",")))+1 :ncol(r_data))]
f_data <- mquant_data[, 1:length(unlist(str_split(information, ",")))]
